...
################################################
#       LINUX X64 BUILDER (Now ARM x64 darwin)  #
################################################
...
# install proj dependencies for arm64-osx
RUN /vcpkg/vcpkg install sqlite3[core,tool] tiff --triplet=arm64-osx

# build proj statically for darwin x64
...
RUN PKG_CONFIG_PATH="/vcpkg/installed/arm64-darwin/lib/pkgconfig" \
    CGO_ENABLED=1 \
    CGO_LDFLAGS='-L/vcpkg/installed/arm64-darwin/lib -g -O2 -static -lstdc++ -lsqlite3 -ltiff -lz -ljpeg -llzma -lm' \
    GOOS=darwin GOARCH=arm64 \
    go build -o ./bin/gocesiumtiler-arm64 \
...
#     go test ./... -v
RUN go test github.com/mfbonfigli/gocesiumtiler/v2/internal/las/golas -run TestLasGeometries -v
...
COPY --from=linux-builder ${PROJECT_FOLDER}/build/bin/gocesiumtiler-arm64 /gocesiumtiler-arm64-osx
